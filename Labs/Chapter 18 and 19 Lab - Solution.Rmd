---
title: "Chapter 18 and 19 Lab - Neural Nets and SVMs - Example Solution"
output:
  pdf_document:
    fig_height: 3
    fig_width: 5
...
Sometimes we get valid predictions. Other times we see a constant (or near constant) prediction being made. 
...
Yes, this one converged in less than 40 iterations. 

Let's try increasing the number of nodes in the hidden layer. Does this neural net converge?

No, this net did not converge. It says it stopped after 100 iterations, no convergence message.

```{r}
set.seed(495)
nnet_carseats2 <- nnet(High ~ . - Sales, train, size = 8, maxit = 400)
```

Try adding the *maxit* parameter to the net with 8 hidden nodes and see if you can find a value for which it converges. 

I was able to get convergence with fewer than 200 iterations. 
...
"Olden et al. 2004 describe a connection weights algorithm that consistently out-performed Garson's algorithm in representing the true variable importance in simulated datasets. This ‘Olden’ method calculates variable importance as the product of the raw input-hidden and hidden-output connection weights between each input and output neuron and sums the product across all hidden neurons. An advantage of this approach is the relative contributions of each connection weight are maintained in terms of both magnitude and sign as compared to Garson's algorithm which only considers the absolute magnitude. For example, connection weights that change sign (e.g., positive to negative) between the input-hidden to hidden-output layers would have a cancelling effect whereas Garson's algorithm may provide misleading results based on the absolute magnitude. An additional advantage is that Olden's algorithm is capable of evaluating neural networks with multiple hidden layers wheras Garson's was developed for networks with a single hidden layer." (olden function help Details section.)

There are several reasons to consider the olden function better described above. 
...
The model with 4 hidden nodes had 56 mistakes on training and 75 mistakes on the test set (both have 200 observations). The model with 8 hidden nodes had 75 mistakes on the training set and 81 on the test set. So, the second model did worse than the first. 

Try one more model with parameters of your choice for at least one new option. How does it do? 

```{r}
set.seed(495)
nnet_carseats3 <- nnet(High ~ . - Sales, train, size = 3, maxit = 400, decay = 0.0001)
...
I chose to drop the number of hidden nodes to 3 and introduce the decay parameter. My third neural net was able to have only 10 mistakes on the training data and 24 on the test set. This is a major improvement from above. The most problematic prediction is still predictions of "No" that should be "Yes". 


Which neural net model do you prefer overall? The third model has the best performance. 
...
```{r}
svm2 <- svm(High ~ . - Sales, data = train, gamma = 0.5, kernel = "poly")
summary(svm2)
```
...
svm1 seems to have 100% accuracy on the training data, but has 71 mistakes on the test set. It may be overfitting - that's a big change from training to test. 
...
Well, my svm2 had 0 AER but 54 mistakes on the test set. So it's better than svm1, but not by much. Will want to see if we can do better. 
...
```{r}
svm3 <- svm(High ~ . - Sales, data = train, gamma = 0.1, kernel = "radial")
summary(svm3)
...
After trying several approaches, I went back to the radial kernel and played with the gamma. A gamma of 0.1 gave me 14 errors on the training data and 29 on the test set. This isn't horrible, compared to the above models. 
...
Answers will vary. There is a lot to sort through. Many examples were classification that I found though. 
