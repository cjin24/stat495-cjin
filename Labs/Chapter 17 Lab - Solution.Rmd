---
title: "Chapter 17 Lab - Random Forests and Boosting - Example Solution"
output:
  pdf_document:
    fig_height: 3
    fig_width: 5
...
First, we create a training and test set to use. 
...
Next, we fit bagging and examine the test set MSE.
...
bagged_estimate <- predict(bagging, newdata = test)


mean((bagged_estimate - test$Salary)^2)
```

Random forests are fit next. I am increasing the number of trees from the default, and will try a few different mtry values, but just show the one I found had the best test MSE. 
...
rf <- randomForest(Salary ~ ., 
                          ...
                          ntree = 1000,
                          mtry = 10, 
                          importance = TRUE) #tried mtry 3, 5, 7, 8, 9, 10, 11

random_forest_estimate <- predict(rf, newdata = test)
mean((random_forest_estimate - test$Salary)^2)
```

Bagging and random forests look very competitive for this. The test MSEs are very close. 

Finally, we perform boosting. I tried a few different parameter choices and this is the model I finally ended up with:
...
boost_estimate <- predict(boost, 
                         newdata = test, 
                         n.trees = 5000)

mean((boost_estimate - test$Salary)^2)
```

RFs and bagging appear to be better in this setting than boosting, at least based on my seed, etc. but not by much. 
